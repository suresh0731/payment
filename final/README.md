...
| Field edit patch | [IDP_Field_Edit_Design.md](./IDP_Field_Edit_Design.md) | `path[]`, catalog, projections, Gate A/B — patch this onto existing `fields` code |
...
| 5 | [IDP_Field_Edit_Design.md](./IDP_Field_Edit_Design.md) | Backend, frontend | **Patch spec** — `path[]` addressing, catalog walker, ingest-column projections, downstream DTO validation before persist. Supersedes `occurrenceIndex` in API §2.5 / LLD §5.5 |
...
| [IDP_Field_Edit_REPO_SCAN_PROMPT.md](./IDP_Field_Edit_REPO_SCAN_PROMPT.md) | **Run before the field-edit patch** — grounds `path[]` / catalog / projector / Gate B in real DTOs and columns |
...
| 2026-08-14 | **Field-edit patch pack.** `occurrenceIndex` cannot address nested objects (`DebitDetails.fx.rate`) or a second array under the same group without a breaking change after go-live. New [IDP_Field_Edit_Design.md](./IDP_Field_Edit_Design.md) locks `path[]` + per-entity catalog + ingest-column projector + shape/downstream validation before persist. Companion [IDP_Field_Edit_REPO_SCAN_PROMPT.md](./IDP_Field_Edit_REPO_SCAN_PROMPT.md) is a discovery-only scan of existing edit/mapper/DDL code before the patch. |
