...
> **Status:** Final v9 — `getUploads`/`getDetail` instruction aggregates; single user review; `performAction` only (optional `/submit`/`/cancel` aliases); entity-only routing
> **Created:** 2026-08-02 · **Corrected:** 2026-08-03 (v2) · 2026-08-03 (v3) · 2026-08-03 (v4) · 2026-08-04 (v6/v7) · **Corrected:** 2026-08-07 (v9)
> **Related:** [README.md](./README.md) · [IDP_LLD.md §9](./IDP_LLD.md#9-code-implementation-strategy) · [IDP_UX_Design.md §8](./IDP_UX_Design.md) · [FSSPaymentsDocIngestion.bpmn](./FSSPaymentsDocIngestion.bpmn) · [../51786-idp-extraction-service/extraction-main](../51786-idp-extraction-service/extraction-main) · [../51786-workflow-management/src/main/java/com/sc/fss/iap/workflow/controller/WorkflowServicesController.java](../51786-workflow-management/src/main/java/com/sc/fss/iap/workflow/controller/WorkflowServicesController.java)

This document lists **all APIs** in the document ingestion & extraction upload flow: what each endpoint does, who calls it, and how it fits the end-to-end journey. Extraction service endpoints are **already built**; gateway upload/action endpoints are **planned**; workflow-management task APIs are **existing** but invoked **server-side by the gateway** for user review actions (portal must not call them directly — see [§3](#3-user-review-action-apis--gateway-facade-required)).

---
## 1. API landscape
| Service | Base path (incl. servlet context path) | Caller | Status |
|---|---|---|---|
| `51786-payment-gateway-service` | `/api/fss/payments/gateway/v1/extraction-uploads` | Portal UI (`paymentMaker`) — upload, list, detail, fields, re-extract, **`performAction`** | **Planned** (new) |
...
> **v9 — list/detail aggregates.** `getUploads` and `getDetail` expose `instructionCount` and overall document **`confidence`** from denormalized meta columns. **`paymentRef` is not on `getUploads`** — payment links on `getDetail.ingestDetails[]` only (modal). See [UX §2.4.1](./IDP_UX_Design.md#241-table-column-rule--instructions-only-no-payment-ref).
...
**Auth:** existing portal JWT. **`paymentMaker`** only for upload, fields, re-extract, and `performAction`.
...
| `POST` | `/fields?uploadId={id}&detailId={detailId}` | User saves draft field edits (one instruction) | Save draft |
| `POST` | `/re-extract?uploadId={id}` | Start a new extraction run | Re-extract button |
| `POST` | `/performAction?uploadId={id}` | User review action — body `{ "action": "SUBMIT"\|"CANCEL", "remarks": "..." }` — **required** (see [LLD §9.4](./IDP_LLD.md#94-extractionuploadactionservice--single-entry-switch-inside-service)) |
...
2. `POST {workflow-management}/v1/start/startWorkflowProcess?processKey=FSS_Payments_Document_Ingestion&referenceId={uploadId}` with process variable `entity`.
...
4. One `FSS_Payments_Document_Ingestion` instance **per extracted PDF**.
...
| --- | --- | --- | --- |
| `uploadId` | string | `fss_payment_upload_meta.id` | Row key, deep link |
...
| `instructionCount` | int | Meta `instruction_count` — denormalized, refreshed on write (§5.1.1) | Instructions column — e.g. `1`, `18` |
| `confidence` | number \| null | Meta `confidence` — overall document LLM extract confidence (`AVG` of per-instruction scores) | Confidence column — see [LLD §6.4](./IDP_LLD.md#64-confidence_score-on-fss_payment_data_ingest_details) |

**Not on list response:** `paymentRef` — modal only ([UX §2.4.1](./IDP_UX_Design.md#241-table-column-rule--instructions-only-no-payment-ref)). **Not in v1:** `lowConfidenceInstructionCount` on list.

**Gateway implementation:** `getUploads` reads **`fss_payment_upload_meta`** only — `instruction_count`, `confidence`; no join to details on poll. Refreshed by `ExtractionUploadAggregateService` after extraction ([LLD §5.1.1](./IDP_LLD.md#511-file-level-aggregates-on-meta-denormalized--not-computed-on-list-read)).
...
| `confidence` | number \| null | Meta `confidence` — overall document LLM confidence |
...
| Field | UI left panel ([UX §4.2](./IDP_UX_Design.md#42-instruction-list-left-panel)) |
| --- | --- |
| `instructionIndex` | Row `#` (`+ 1` in UI) |
| `txRef` | TxRef column |
| `subActivityId` | SubAct — group/filter (Redemption, Subscription, …) |
| `activityId` / `trnTyp` | BT / OTT badge |
| `valueDate`, `debitAmount` | Optional columns |
| `confidenceScore`, `lowConfidenceFieldCount` | Conf + ⚠ |
| `paymentId`, `paymentRef` | Post-handoff payment link (per instruction when `COMPLETED`) |
| `extractedData` | Right panel field grids — **one instruction only** |

**Grouping:** API returns a **flat** `ingestDetails[]`. The modal left panel groups or filters by `subActivityId` / `trnTyp` **client-side** — no `GET .../getDetailGrouped` endpoint in v1.

`extractedData` is the per-instruction `{ header, initiationDetail }` JSON (single object, not array).
...
Saves user edits for **one instruction** (`detailId` = `fss_payment_data_ingest_details.id`). Re-denormalizes summary columns on save.
...
**Allowed when:** meta `status` is `READY_FOR_REVIEW` or `FAILED`, caller is `paymentMaker`. Resets all detail rows: `retry+1`, clear `extracted_data`/`error_desc`, `status=PROCESSING`; re-fire extraction.

**Response `202 Accepted`:** `{ "uploadId": "...", "status": "PROCESSING" }`

---

## 3. User review action APIs — gateway façade (required)
...
| --- | --- | --- | --- |
| `POST` | `/performAction?uploadId={id}` | User (`paymentMaker`) | Per `action` in body — see [LLD §9.4](./IDP_LLD.md#94-extractionuploadactionservice--single-entry-switch-inside-service)) |
...
| View payment | Open row → `getDetail` → **View payment** on `ingestDetails[]` with `paymentId` / `paymentRef` (modal only — not list table) | - |
